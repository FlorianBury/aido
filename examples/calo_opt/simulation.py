...
class Simulation():
    ...
    def stitch_one_event(self,mfs):
        ...
        part_type = torch.concatenate(
            [
                torch.from_numpy(mf['particle_type']).squeeze(1)
                for mf in mfs
            ],
            axis = -1,
        )
        ...
        Es = np.array([mf['sensor_energy'] for mf in mfs])
        Es = torch.from_numpy(Es)
        E = Es.sum(dim=0)
        fracs = torch.nan_to_num(Es / (E+1e-10))
    ...
    def stitch_events(self,mf):
        ...
        for _ in range(self.n_events):
            N = max(1,np.random.poisson(self.mean_poisson))
            indices = np.random.choice(len(mf), size=N, replace=False)
            events.append(
                self.stitch_one_event(
                    [
                        MiniFrame(
                            {
        ...
        return CaloGraphDataset(events)
